...
def load_data(symbol: str) -> pd.DataFrame:
    """Load market data for symbol from CSV."""
    import os
...
def _rsi_label(rsi: float | None) -> str:
    """Get RSI label and interpretation."""
    if rsi is None:
        return "N/A"
    if rsi >= 70:
        return f"{rsi} (Overbought — pullback possible)"
    if rsi <= 30:
        return f"{rsi} (Oversold — bounce possible)"
    return f"{rsi} (Neutral)"


def get_market_data(symbol: str) -> dict:
    """Get complete market data and analysis for symbol."""
    symbol = symbol.upper().strip()
    try:
        df = load_data(symbol)
